...
class AgentRunner:
  ...
  async def spawn_sub_agent(
    ...
    *,
    ...
    sub_log = EventLog(
      on_event=getattr(self._log, "_on_event", None),
    ...
    sub_runner = AgentRunner(
      ...
      per_turn_timeout=self._per_turn_timeout,
  ...
  async def run(
    ...
    while True:
      ...
      async def _do_stream() -> None:
        ...
        nonlocal current_signature, current_text_block
      ...
      try:
        if self._per_turn_timeout is not None:
          await asyncio.wait_for(_do_stream(), timeout=self._per_turn_timeout)
        else:
          await _do_stream()
      except asyncio.TimeoutError:
        log.error("[%s] Turn %d timed out after %.0fs", self._sid, turn_count, self._per_turn_timeout)
        self._append({"type": "error", "error": f"Turn timed out after {self._per_turn_timeout}s"})
        return
      except Exception as exc:
        log.error("[%s] Stream error on turn %d after %.1fs: %s", self._sid, turn_count, time.time() - turn_t0, exc)
