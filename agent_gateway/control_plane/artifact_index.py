...
# so repeated lookups hit the research repository once per distinct id instead of
# once per sidecar, and reset after the call so mutable repository state is never
# reused across requests. None when no list call is active (lookups go direct).
_research_classification_cache: "contextvars.ContextVar[dict[int, dict[str, Any] | None] | None]" = (
  contextvars.ContextVar("_research_classification_cache", default=None)
)
...
def _research_file_classification(*, user_id: str, research_file_id: int | None) -> dict[str, Any] | None:
  if research_file_id is None:
    return None
  cache = _research_classification_cache.get()
  if cache is not None and research_file_id in cache:
    return cache[research_file_id]
  result = _load_research_file_classification(user_id=user_id, research_file_id=research_file_id)
  if cache is not None:
    cache[research_file_id] = result
  return result


def _load_research_file_classification(*, user_id: str, research_file_id: int) -> dict[str, Any] | None:
  try:
...
def _raw_sidecar_payload_cached(path_str: str, mtime_ns: int) -> dict[str, Any] | None:
  """Read + json.load an artifact sidecar, cached by (path, mtime_ns).

  Caches only the file read + parse — a pure function of file content (keyed by
  mtime_ns, so a changed file gets a new key; the LRU bounds memory). It does
  NOT classify the payload: effective visibility/origin via
  `_effective_artifact_fields` reads mutable research-repository state and must
  be recomputed fresh on every call (else changed classifications would be
  served stale until the sidecar mtime changes). The control artifacts list
  re-reads every sidecar in the per-user tree on each call (can be thousands),
  so caching the parse alone removes the dominant file-read+parse cost while
  keeping classification correct. Returns None for missing/unreadable/non-dict.

  The returned dict is shared with the cache; callers must copy it before any
  mutation and must never mutate it in place.
  """
  try:
...
def _artifact_response_from_sidecar(
  *,
  ...
  try:
    ...
    return None
  cached_payload = _raw_sidecar_payload_cached(str(path), mtime_ns)
  if cached_payload is None:
    return None
  payload = dict(cached_payload)  # shallow copy: never mutate the cached dict
  payload.update(_effective_artifact_fields(payload, user_id=user_id))
  if not _payload_matches_artifact_filters(payload, filters=filters):
    return None
...
def artifact_responses_for_user(
  *,
  ...
  # Memoize research-file classification for the duration of this list call (see
  # _research_classification_cache): a single response can classify hundreds of
  # sidecars referencing only a few dozen distinct research files. Request-scoped
  # (reset in finally) so mutable repository state is never reused across calls.
  token = _research_classification_cache.set({})
  try:
    return _artifact_responses_for_user_impl(
      request=request,
      user_id=user_id,
    ...
    _research_classification_cache.reset(token)


def _artifact_responses_for_user_impl(
  *,
  request: Request,
  user_id: str,
  ticker_filter: str | None,
  ...
  for _mtime_ns, path, path_ticker, path_skill, artifact_id in _artifact_sidecars_for_user(
    ...
    if artifact is None:
      continue
    if not _artifact_matches_optional_run_filter(artifact, run_id_filter, run_windows):
      continue
  ...
  return [artifact for _mtime_ns, _path, artifact in artifact_entries[:effective_limit]]


def _artifact_matches_optional_run_filter(
  artifact: ArtifactResponse,
  run_id_filter: str | None,
  run_windows: dict[str, float],
) -> bool:
  if run_id_filter is None:
    return True
  if artifact.run_id != run_id_filter:
    return False
  return _artifact_matches_run_window(
    artifact,
