...
def test_fixture_surfaces_fail_closed_when_environment_is_ambiguous(monkeypatch, tmp_path) -> None:
  ...
  with pytest.raises(ValueError, match="dev-only"):
    _resolve_provider("fixture", None, None, None, None)

  with pytest.raises(ValueError, match="dev-only"):
    SkillLoader(_skills_root()).load("fixture-sleep")
  with pytest.raises(ValueError, match="dev-only"):
    SkillLoader(_skills_root()).load(FIXTURE_HTML_ARTIFACT_SKILL_NAME)
  with pytest.raises(ValueError, match="dev-only"):
    SkillLoader(_skills_root()).load(FIXTURE_APPROVAL_HTML_ARTIFACT_SKILL_NAME)
  with pytest.raises(ValueError, match="dev-only"):
...
def test_fixture_surfaces_refuse_production_even_with_dev_flags(monkeypatch, tmp_path) -> None:
  ...
  with pytest.raises(ValueError, match="Unsupported provider 'fixture'"):
    AuthConfig.from_dict({"provider": "fixture", "billing_mode": "byok", "model": FIXTURE_MODEL_ID})
...
def test_fixture_surfaces_available_in_explicit_test_environment(monkeypatch) -> None:
  ...
  provider, provider_name, config = _resolve_provider("fixture", None, None, None, None)

  assert provider_name == "fixture"
  assert provider.name == "fixture"
  assert config["model"] == FIXTURE_MODEL_ID
  assert SkillLoader(_skills_root()).load("fixture-sleep").name == "fixture-sleep"
  html_fixture = SkillLoader(_skills_root()).load(FIXTURE_HTML_ARTIFACT_SKILL_NAME)
  assert html_fixture.name == FIXTURE_HTML_ARTIFACT_SKILL_NAME
  assert html_fixture.scope == "ticker"
  assert html_fixture.metadata is not None
  assert html_fixture.metadata["catalog"] is False
  assert html_fixture.agent_callable is False
  ...
  approval_fixture = SkillLoader(_skills_root()).load(FIXTURE_APPROVAL_HTML_ARTIFACT_SKILL_NAME)
  assert approval_fixture.name == FIXTURE_APPROVAL_HTML_ARTIFACT_SKILL_NAME
  ...
  assert load_profile("_fixture").provider == "fixture"
